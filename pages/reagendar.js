...
function Valor(props) {
  ...
  const sendAgendamento = async e => {
    ...
    if((datavalida[0] < dataAtual) || (datavalida_2[0] > datavalida[0])){
      ...
      try {
        const res = await fetch(`http://localhost:3030/Servicos/atualiza/${props.pet_reagendar.id_pet}`, {
          method: 'PATCH',
        ...
        setResponse({
          formSave: false,
          type: 'error',
          message: "Erro: Falha ao reagendar!"
        });
      }
    }
        ...
        <Container className="imgLogin">
          ...
          <div className="col-md-8 offset-2 divMain2" >
            <div className="bordinha">
              <div className="top">
                <h3>Reagendar horário do pet </h3>
                ...
                <Form onSubmit={handleSubmit(sendAgendamento)} noValidate>
                  <Row>
                    <Col className="col-md-10 offset-1">
                      ...
                      <Input className="form-control mr-sm-2" type="text" name="title" id="title" value={props.pet_reagendar.NOME_PET} disabled placeholder="Nome Pet:" onChange={onChangeInput} />
                    </Col>
                  </Row>
                  <Row>
                    <Col className="col-md-10 offset-1">
                  ...
                  <br />
                  <Row>
                    <Col className="col-md-10 offset-2">
                      <Button color="primary" type="submit">Reagendar</Button>{' '}
                    </Col>
                  </Row>
                </Form>
              </Container>
...
export async function getServerSideProps(ctx) {
  ...
  const { id_pet_reagendar } = parseCookies(ctx)
  var pet_reagendar = JSON.parse(id_pet_reagendar)
  console.log(pet_reagendar)
  return {
    props: {pet_reagendar}
  };
}